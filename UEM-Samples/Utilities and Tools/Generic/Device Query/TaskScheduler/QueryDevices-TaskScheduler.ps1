...
Function Invoke-DeviceQuery {
    ...
    [HashTable]$header
        ...
        try {
            foreach ($devID in $id){ 
                $ServicePoint = [System.Net.ServicePointManager]::FindServicePoint($WSOServer)
                Invoke-WebRequest -Uri "$WSOServer/API/mdm/devices/$devID/commands?command=DeviceQuery" -Headers $header -Method 'POST'
                $ServicePoint.CloseConnectionGroup("")
            }
        } catch {
            Write-Host "An error occurred when logging on $_"
    ...
    function Invoke-AutoDeviceQuery {
        Get-Environment 
        ...
        Invoke-DeviceQuery -DeviceId $id -WSOServer $WSOServer -header $header
    }


    Invoke-AutoDeviceQuery
