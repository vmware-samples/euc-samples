...
def getdeviceinfo():
    deviceinfo = {}
    try:
        fileName = '/tmp/SPHardwareDataType.plist'
        t = subprocess.check_output(['/usr/sbin/system_profiler', '-xml', 'SPHardwareDataType', '>', fileName])
...
def init_registration(consoleuser, username=True, email=False):
    ...
    runcmd('sudo', '-u', consoleuser, 'defaults', 'write', 'menu.nomad.DEPNotify', 'pathToPlistFile',
           '/Users/Shared/UserInput.plist')
    runcmd('sudo', '-u', consoleuser, 'defaults', 'write', 'menu.nomad.DEPNotify', 'registrationButtonLabel',
           'Continue')

    if username:
        runcmd('sudo', '-u', consoleuser, 'defaults', 'write', 'menu.nomad.DEPNotify', 'registrationMainTitle',
               'Enter your username')
        # runcmd('sudo', '-u', consoleuser, 'defaults', 'write', 'menu.nomad.DEPNotify', 'textField1Placeholder',
        # 'johndoe')
        runcmd('sudo', '-u', consoleuser, 'defaults', 'write', 'menu.nomad.DEPNotify', 'textField1Label', 'Username')
        runcmd('sudo', '-u', consoleuser, 'defaults', 'write', 'menu.nomad.DEPNotify', 'textField1RegexPattern',
               '[A-Z0-9a-z.-_@]')
...
def main():
    ...
    o.add_option('--enrolled-username', default=None,
                 help='Enter the name the device is enrolled with')
    ...
    if opts.enrolled_username:
        enrolledusername = opts.enrolled_username
    ...
    deviceosversion = str(deviceinfo['osvers'])
    ...
    if not opts.sideload_mode:
        ...
        elif opts.enrolled_username:
            oslog('Warning - Neither --sideload-mode, --prompt-username, or --prompt-email used.')
            oslog('Warning - However - Username supplied')
            oslog('Using %s for enrollment username' % opts.enrolled_username)
            username = str(opts.enrolled_username)
        ...
        if not opts.prompt_email:
            oslog('%s/api/system/users/search?username=%s' % (apiurl, username))
        ...
        regtoken = response.replace('\'', '').replace('b', '').replace('\"', '')
        ...
        try:
            ...
            req = urllib.request.Request(url, data=data, headers=agent_header)
            response = urllib.request.urlopen(req).read()
    ...
    if opts.wsone:
        ...
        oslog('POST - ' + url)
        try:
            req = urllib.request.Request(url, data=str.encode(""), headers=ORIGIN_HEADERS)
            response = urllib.request.urlopen(req).read()
            oslog('Raw Response - ' + str(response))
        except IOError as e:
            if hasattr(e, 'reason'):
    ...
    if deviceosversion >= '11.0':
        ...
        runcmd('sudo', '-u', consoleuser, '/usr/bin/open', '-b', 'com.apple.systempreferences',
               '/System/Library/PreferencePanes/Profiles.prefPane')
    elif deviceosversion >= '10.15.1':
        runcmd('sudo', '-u', consoleuser, '/usr/bin/open', '-a', '/System/Applications/System Preferences.app',
               enrollmentProfilePath)
    else:
        runcmd('sudo', '-u', consoleuser, '/usr/bin/open', '-a', '/Applications/System Preferences.app',
               enrollmentProfilePath)
    ...
    while runcount < 180:
        profilelist = str(runcmd('sudo', '/usr/bin/profiles', '-vP'))
    ...
    if enrolled:  # enrolled = True - Tell the user enrollment succeeded and clean up
        ...
        if opts.prompt_for_restart:
            ...
            depnotify('Command: WindowStyle: Activate')
            depnotify(
                'Status: Your computer will automatically restart in %s seconds. Please login normally to complete the migration.' % str(
                    delay))
